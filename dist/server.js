...
if (process.env.RUNNING_IN_HEROKU) {
    ...
    let server = http.createServer(app).listen(app.get("port"), () => {
        onListening(false, app.get("env"), server.address());
    });
}
else {
    ...
    let server = https.createServer(httpsOptions, app).listen(app.get("port"), () => {
        onListening(true, app.get("env"), server.address());
    });
}
// Helper to log status after Express starts
function onListening(isHttps, mode, addressInfo) {
    let scheme = isHttps ? "https" : "http";
    console.log(" Salesforce Marketing Cloud demo at Dreamforce 2018.");
    console.log((" Express is running '%s' server in '%s' mode at %s://[%s]:%d, family: %s"), scheme, mode, scheme, addressInfo.address, addressInfo.port, addressInfo.family);
